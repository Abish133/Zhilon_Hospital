...
async function main() {
  ...
  try {
    ...
    await RadiologyReports.create({ rad_order_id:ro1.rad_order_id, findings:'Cardiothoracic ratio is 0.52 (upper limit of normal).\nBoth lung fields are clear; no focal consolidation, mass lesion or pleural effusion seen.\nMediastinal contour is normal. Trachea is central.\nVisualised bony thorax and soft tissues are unremarkable.\nBoth costophrenic and cardiophrenic angles are clear.', impression:'Borderline cardiomegaly. No active cardiopulmonary disease.\nSuggest clinical correlation with ECG / 2D-Echo if cardiac enlargement is suspected.', reported_by:dRajesh.id, reported_at:now, report_url:null, status:'Approved', hospital_id:H, is_active:true },{ transaction:t });
    ...
    await RadiologyReports.create({ rad_order_id:ro2.rad_order_id, findings:'Uterus is bulky, measuring 9.2 x 5.4 x 4.1 cm, with a well-defined intramural fibroid (3.2 x 2.8 cm) along the posterior wall.\nEndometrial thickness is 8 mm, appropriate for the cycle phase.\nBoth ovaries are normal in size and echotexture; no adnexal mass identified.\nNo free fluid in the pouch of Douglas.\nLiver, gall bladder, both kidneys and spleen appear normal.', impression:'Bulky uterus with a posterior wall intramural fibroid (~3.2 cm).\nRecommend gynaecology review and a follow-up ultrasound in 6 months.', reported_by:dRajesh.id, reported_at:dt(-1,13), report_url:null, status:'Approved', hospital_id:H, is_active:true },{ transaction:t });
    ...
    const ro_v8 = await RadiologyOrders.create({ patient_id:pLakshmi.patient_id,  uhid:pLakshmi.uhid,  visit_type:'OPD', visit_id:v8.visit.visit_id, rad_test_id:getRad('RAD-USG-AB').rad_test_id, test_name:'USG Neck', modality:'USG', clinical_info:'Thyroid Evaluation', ordered_by:dAnita.id, order_date:now, scheduled_date:today, scheduled_time:'10:00:00', status:'Completed', hospital_id:H, is_active:true },{ transaction:t });
    await RadiologyImaging.create({ rad_order_id:ro_v8.rad_order_id, imaging_date:now, technologist_id:empMap.Radiologist.employee_id, technical_notes:'High-resolution USG of thyroid and neck performed.', hospital_id:H, is_active:true },{ transaction:t });
    console.log(`  ✔ Radiology Orders (6) + Imaging (3) + Reports (2)`);
